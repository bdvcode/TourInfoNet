	...
	<div id="guidedContent">
		<div class="guidedContent_inner">
			...
			<div class="guided_header afterclr">
				<div class="guided_header_cell col1"><span>Miejsce</span></div>
				<div class="guided_header_cell col2"><span>Szlak</span></div>
				<div class="guided_header_cell col3"><span>Kontakty</span></div>
			</div><!-- guided_header -->
			<?
							...
							<?if(CModule::IncludeModule("iblock")):
								...
								while($ar_result = $db_list->GetNext()):?>
									<?if(($ar_result["ID"])==$countrId):?>
										<?echo($ar_result["UF_PL_NAME"]);?>,
									<?endif;?>
								<?endwhile;?>
							<?endif;?>
							<?$cityId=$arItem["PROPERTIES"]["city"]["VALUE"];?>
							<?$db_props = CIBlockElement::GetProperty(9, $cityId, "sort", "asc", Array("CODE"=>"NAME_PL"));if($ar_props = $db_props->Fetch()):echo ($ar_props["VALUE"]); endif;?>
						</div></div>
					<div class="guided_item_cell col2">
						<div>
							<div class="guided_item_title"><?=$arItem["PROPERTIES"]["name_pl"]["VALUE"]?></div>
							<div class="guided_item_preview"><?=htmlspecialcharsBack($arItem["PROPERTIES"]["desc_pl"]["VALUE"]["TEXT"])?></div>
						</div>
					</div>
					<div class="guided_item_cell col3">
						<div>
							<div class="guided_item_cont_title"><?=$arItem["PROPERTIES"]["org_pl"]["VALUE"]?></div>
