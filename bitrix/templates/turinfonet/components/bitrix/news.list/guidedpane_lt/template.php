	...
	<div id="guidedContent">
		<div class="guidedContent_inner">
			...
			<div class="guided_header afterclr">
				<div class="guided_header_cell col1"><span>Vieta</span></div>
				<div class="guided_header_cell col2"><span>Maršrutas</span></div>
				<div class="guided_header_cell col3"><span>Kontaktai</span></div>
			</div><!-- guided_header -->
			<?
			...
			if(($paramValueM - 1) > 0){
				...
				echo sprintf('<a href="%s">Pirmyn</a>',
					$urlM);
			}
			?>
							...
							<?if(CModule::IncludeModule("iblock")):
								...
								while($ar_result = $db_list->GetNext()):?>
									<?if(($ar_result["ID"])==$countrId):?>
										<?echo($ar_result["UF_LT_NAME"]);?>,
									<?endif;?>
								<?endwhile;?>
							<?endif;?>
							<?$cityId=$arItem["PROPERTIES"]["city"]["VALUE"];?>
							<?$db_props = CIBlockElement::GetProperty(9, $cityId, "sort", "asc", Array("CODE"=>"NAME_LT"));if($ar_props = $db_props->Fetch()):echo ($ar_props["VALUE"]); endif;?>
						</div></div>
					<div class="guided_item_cell col2">
						<div>
							<div class="guided_item_title"><?=$arItem["PROPERTIES"]["name_lt"]["VALUE"]?></div>
							<div class="guided_item_preview"><?=htmlspecialcharsBack($arItem["PROPERTIES"]["desc_lt"]["VALUE"]["TEXT"])?></div>
						</div>
					</div>
					<div class="guided_item_cell col3">
						<div>
							<div class="guided_item_cont_title"><?=$arItem["PROPERTIES"]["org_lt"]["VALUE"]?></div>
